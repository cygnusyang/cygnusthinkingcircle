...
| REQ-003 | `/posts/` 列表页分页或两级浏览 | P0 | 小 | ✅ 已完成 |
| REQ-004 | 文章文件命名与 URL 规范化 | P1 | 中 | ❌ 暂缓（保持现状） |
| REQ-005 | 启用 Hugo 分类法（tags/categories） | P1 | 中 | ✅ 已完成 |
| REQ-006 | 侧边栏导航按需渲染 | P1 | 中 | ✅ 已完成 |
| REQ-007 | 内容创建管道自动化 | P1 | 中 | ✅ 已完成 |
| REQ-008 | 启用输出压缩（minify） | P2 | 小 | ✅ 已完成 |
| REQ-009 | 统一目录命名语言（URL 美化） | P2 | 中 | ❌ 暂缓（保持现状） |
| REQ-010 | 相关文章推荐优化 | P2 | 小 | ✅ 已完成 |
| REQ-011 | 图片处理与资源管道 | P2 | 中 | ✅ 已完成（基础配置） |
| REQ-012 | CI/CD 自动化构建与校验 | P2 | 中 | ✅ 已完成 |
...
| 需求 | 说明 |
|------|------|
| REQ-004 | 文件命名规范化 — 保持现状，URL 含中文不影响功能 | ❌ 暂缓 |
| REQ-009 | 目录英文名统一 — 保持现状，robots.txt 已配置完善 | ❌ 暂缓 |
| REQ-006 | 侧边栏按需渲染 — 减少每页 HTML 体积 | ✅ |
| REQ-011 | 图片处理管道 — imaging 基础配置已添加 | ✅ |
